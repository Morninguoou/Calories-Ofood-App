import 'dart:convert';
import 'package:http/http.dart' as http;
import 'package:projectapp/models/NutritionData.dart';
import 'package:projectapp/models/planindatelistPage.dart';
import 'package:projectapp/models/recommendplan.dart'; // Import the Planner model

class PlanAppRecommendDetailService {
  static Future<List<NutritionData>> getFoodRecommendationDetail(String plannerID) async {
    try {
      final response = await http
          .get(Uri.parse('http://10.0.2.2/FoodRecommendation/$plannerID'))
          .timeout(Duration(minutes: 2)); // Set a timeout of 2 minutes

      if (response.statusCode == 200) {
        List<dynamic> data = json.decode(response.body);
        return data
            .map((dataJson) => NutritionData.fromJson(dataJson))
            .toList();
      } else {
        throw Exception(
            'Failed to load planners. Status code: ${response.statusCode}');
      }
    } catch (e) {
      print('Exception occurred: $e');
      throw Exception('Failed to load planners');
    }
  }
}